...
- **Version 1.4.0 - Player Composition & Inheritance Refactor**:
  - **Weapon Decoupling**: Completely extracted weapon logic (Blaster, Orbitals, Nova, Shockwave, Missiles, Laser Cannon) from `Player.js` into independent modular classes inside `js/entities/player/weapons/`.
  - **BasePlayer Architecture**: Extracted core movement, physics, stats, collisions, and UI linking into `BasePlayer.js`. `Player.js` now simply extends `BasePlayer` and overrides `initWeapons()`.
  - **Modular UI Support**: Multi-tree UI support added to the Laboratory window (Main, Class, Weapon trees) in preparation for multiple ship classes.
...
- **60 FPS Performance Optimization Architecture**:
  ...
  - **BitmapText Cache (`Pool.js`)**: Migrated `FloatingTextPool` from `PIXI.Text` to `PIXI.BitmapText`. Generates hidden font atlases at startup (`DamageFont` and `DamageFontCrit` with baked drop shadows) and tints them via GPU to eliminate CPU canvas rasterization overhead during massive AoE combat.
  - **WeakMap for Hit Cooldowns**: Piercing weapons like `NovaProjectile` utilize `WeakMap` to track enemy hit cooldowns (`this.hitCooldowns`). This prevents severe memory leaks by allowing the Garbage Collector to clean up unpooled `Enemy` instances immediately upon death without lingering references.
