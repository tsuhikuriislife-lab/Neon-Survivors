...
export const upgradeDatabase = [
  ...
  },
  {
    ...
    isAvailable: (p) => Math.round((p.autoMagnetChance || 0) * 100) < 100,
    apply: (p) => {
      p.autoMagnetChance = Math.min(1, Math.round(((p.autoMagnetChance || 0) + 0.10) * 100) / 100);
  ...
  },
  {
    ...
    desc: 'Launches 2 missiles that hunt enemies and deal AoE damage.',
  ...
  },
  {
    ...
    desc: '+2 Missiles.',
    isAvailable: (p) => p.weapons.missiles.level > 0 && (p.weapons.missiles.countUpgrades || 0) < 5,
    apply: (p) => {
